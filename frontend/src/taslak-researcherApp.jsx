...
import { AlertTriangle, Map as MapIcon, Camera, Activity, Moon, LogOut, ArrowLeft } from 'lucide-react';
      ...
      <div className="min-h-screen bg-[#050b14] flex items-center justify-center p-4">
        <div className="bg-slate-900 p-8 rounded-3xl w-full max-w-md border border-indigo-500/30">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            <input type="text" placeholder="Adınız (Örn: Ayse)" value={username} onChange={e=>setUsername(e.target.value)} className="w-full p-3 bg-slate-800 rounded-xl text-white outline-none" required />
            <input type="password" placeholder="Şifreniz (Örn: arastirmaci123)" value={password} onChange={e=>setPassword(e.target.value)} className="w-full p-3 bg-slate-800 rounded-xl text-white outline-none" required />
  ...
  return (
    <div className="flex min-h-screen bg-[#050b14] text-white">
      <aside className="w-64 border-r border-slate-800 p-6 flex flex-col">
        <h2 className="text-xl font-bold text-indigo-400 mb-8">Researcher</h2>
        <nav className="flex-1 space-y-2">
          {[{id:'risk', label:'Risk Durumu', icon:AlertTriangle}, {id:'map', label:'Harita', icon:MapIcon}, {id:'cameras', label:'Kameralar', icon:Camera}, {id:'history', label:'Tarihsel Analiz', icon:Activity}, {id:'moon', label:'Ay Durumu', icon:Moon}].map(item => (
      ...
      <main className="flex-1 p-8">
        <h1 className="text-2xl font-bold mb-4">Hoş geldin Dr. {auth.name}</h1>
        <div className="bg-slate-900 border border-slate-800 p-8 rounded-2xl">
          {activeTab} paneli içeriği yükleniyor...
        </div>
      </main>
    </div>
  );

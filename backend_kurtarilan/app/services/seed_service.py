...
from app.models import MoonPosition, Port, Station, User
...
def seed_stations(db: Session) -> None:
    ...
    # Pre-load existing ports
    existing_ports_by_name = {
        port.name: port
        for port in db.scalars(select(Port)).all()
    }

    # Artık tüm istasyonları deaktive etmiyoruz. 
    # Sadece varsayılan olanların aktif olduğundan emin oluyoruz.
    for station_payload in DEFAULT_STATIONS:
        # Ensure Port exists
        port_name = station_payload["name"]
        port = existing_ports_by_name.get(port_name)
        if port is None:
            port = Port(
                name=port_name,
                ...
                description=f"{port_name} otomatik oluşturuldu.",
                is_active=True,
            )
            db.add(port)
            db.flush()
            existing_ports_by_name[port_name] = port
